...
GraphEditor::GraphEditor(String Title, String Name) : SettingsListenerComponent(Name)
{
    title = Title;
    max = 0;
...
void GraphEditor::paint (Graphics& g)
{
    ...
    if (selected != nullptr) *selected = screenToData(getMouseXYRelative());
...
void GraphEditor::setMinMax() {
    ...
    for (const auto & d : data) {
        if (d.Value * 1.2 > max) max = d.Value * 1.2;
        if (d.Value * 0.8 < min) min = d.Value * 0.8;
    }
}
