...
void ProgressWatcher::BreedComplete(const BreedCompleteData & data) {
    ...
    if (type == "Frame") progress.getReference(data.ident) = data.framesComplete;
    else if (type == "Timbre") progress.getReference(data.ident) = data.timbreMetric;
    else if (type == "Panning") progress.getReference(data.ident) = data.panningMetric;
    triggerAsyncUpdate();
}
