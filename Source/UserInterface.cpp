...
UserInterface::UserInterface(XmlElement * e, Settings * s)
{
    forEachXmlChildElement(*e, c) {
        if (c->getTagName() == "FrequencyGraph") LoadDisplay(c, new FrequencyGraph());
        else if (c->getTagName() == "PanningDisplay") LoadDisplay(c, new PanningDisplay());
        else if (c->getTagName() == "ProgressWatcher") LoadDisplay(c, new ProgressWatcher(c->getStringAttribute("type")));
        else if (c->getTagName() == "ProgressLine") LoadDisplay(c, new ProgressLine(s));
        
        else if (c->getTagName() == "ValueEditor") LoadInterface(c, new ValueEditor(c->getStringAttribute("title"),
                                                                                    c->getStringAttribute("track")));
        else if (c->getTagName() == "GraphEditor") LoadInterface(c, new GraphEditor(c->getStringAttribute("title"),
                                                                                      c->getStringAttribute("track")));
        else if (c->getTagName() == "EQCurveEditor") LoadInterface(c, new EQCurveEditor(c->getStringAttribute("title"),
                                                                                        c->getStringAttribute("track")));
        else if (c->getTagName() == "AudioBin") LoadAudioBin(c, new AudioBin(c->getStringAttribute("title"),
                                                                             c->getStringAttribute("track")));
        else if (c->getTagName() == "TargetEditor") LoadTargetEditor(c, new TargetEditor(c->getStringAttribute("title"),
                                                                                         c->getStringAttribute("track")));
    }
    LinkInternals();
}
...
void UserInterface::AddSettingsListeners(Settings *settings) {
    for (auto & i : interfaces) settings->AddListener(i->Interface);
    for (auto & i : targetEditors) settings->AddListener(i->Interface);
    for (auto & i : audioBins) settings->AddListener(i->Interface);
}
